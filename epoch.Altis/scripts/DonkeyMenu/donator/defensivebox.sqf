...
// Item to place
_ItemsClassName							= "Box_NATO_Wps_F";	// Classname of the item to place
_ItemDisplayName						= "Base Defense Package";				// Item name to display in the player menu

// Materials Required to Create Lights
_EnergyRequired							= 200;						// Amount of energy expended to build (0 for zero energy required)
...
if(_hasEnough && _hasEnergy && _CanPlace && _FriendlyInRange) then
{
	...
	// Epoch specific stuff
	_object addItemCargoGlobal  ["FAK", 10];
	_object addItemCargoGlobal  ["Towelette", 10];
	...
	cutText [format["Spawned lights DO NOT SAVE after server restart!"], "PLAIN DOWN"];	
	_ObjectPlaced = true;
}
else
{
